...
public class Blosum {
  ...
  private HashMap<char,HashMap<char, int>> blosumMap;

  public Blosum() {
    blosumMap = new HashMap<char,HashMap<char, int>>();
    for(int i = 0; i < blosumMatrix.length; i++) {
      HashMap<char, int> map = new HashMap<char, int>();
      for(int j = 0; j < blosumMatrix.length; j++) {
        map.put(blosumIndex[j], blosumMatrix[i][j]);
      }
      blosumMap.put(blosumIndex[i], map);
    }
  }
